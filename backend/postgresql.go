...
func (p *PostgresBackend) Save(ctx context.Context, r *TaskRecord) error {
	...
	_, err = p.db.ExecContext(ctx, `
        INSERT INTO tasks (id, name, args, output, status, queue, config, created_at, started_at, completed_at, retried_at, duration_ms)
        ...
        ON CONFLICT (id) DO UPDATE SET
			...
			retried_at   = CASE WHEN EXCLUDED.completed_at != '0001-01-01' THEN EXCLUDED.completed_at ELSE tasks.completed_at END,
...
func (p *PostgresBackend) Get(ctx context.Context, id string) (*TaskRecord, error) {
	...
	row := p.db.QueryRowContext(ctx, `
        SELECT id, name, args, output, status, queue, config, created_at, started_at, completed_at, retried_at, duration_ms
